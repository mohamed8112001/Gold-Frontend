...
const Dashboard = () => {
        ...
        <CardContent className="p-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              {description && (
                <p className="text-xs text-secondary-800 mt-1">{description}</p>
              )}
            </div>
            <div
              className={`w-10 h-10 bg-primary-500 rounded-full flex items-center justify-center shadow-md`}
            >
  ...
  const ActivityItem = ({ activity }) => {
    const getIcon = () => {
      switch (activity.type) {
        case "booking":
          return <Calendar className="w-4 h-4 text-[#C37C00]" />;
        case "favorite":
          return <Heart className="w-4 h-4 text-[#C37C00]" />;
        case "review":
          return <Star className="w-4 h-4 text-[#C37C00]" />;
        default:
          return <User className="w-4 h-4 text-[#C37C00]" />;
      }
    };
    ...
    const getStatusColor = () => {
      switch (activity.status) {
        case "confirmed":
          return "text-[#8A5700] bg-[#FFDB99]";
        case "pending":
          return "text-[#C37C00] bg-[#FFF0CC]";
        case "completed":
          return "text-[#A66A00] bg-[#FFE6B3]";
        default:
          return "text-[#B8850A] bg-[#FFF8E6]";
      }
    };
  ...
  const BookingItem = ({ booking }) => (
    <Card className="hover:shadow transition-all duration-300 bg-white/90 backdrop-blur-md rounded-lg">
      <CardContent className="p-4">
        <div className="flex items-center justify-between">
          <div>
            <h4 className="font-semibold text-[#6D552C]">
              {booking.shop?.name || booking.shopName || "متجر غير معروف"}
            </h4>
            <div className="flex items-center space-x-2 text-sm text-[#8A6C37] mt-1">
              <Calendar className="w-4 h-4" />
              <span>
                {booking.date || booking.appointmentDate || "تاريخ غير معروف"}
              </span>
              <Clock className="w-4 h-4" />
              <span>
                {booking.time || booking.appointmentTime || "وقت غير معروف"}
              </span>
            </div>
            <p className="text-xs text-[#92723A] mt-1">
              {booking.type || booking.serviceType || "خدمة عامة"}
            </p>
          </div>
              ...
              booking.status !== "completed" && (
                <div className="flex space-x-1">
                  <Button
                    ...
                    className="border-[#C37C00] text-[#C37C00] hover:bg-[#FFF8E6] rounded-lg"
                    onClick={() =>
                      navigate(ROUTES.EDIT_BOOKING(booking._id || booking.id))
                    }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <StatCard
          ...
          description={`${stats.activeBookings || 0} حجز نشط`}
          color="blue"
        />
        <StatCard
          icon={Star}
          title="التقييمات"
          value={stats.reviews}
          description="التقييمات المكتوبة"
          color="green"
        />
        {isShopOwner && (
          <>
      ...
      <div className="space-y-4">
        {bookings.length > 0 ? (
          ...
          <div className="text-center py-12 text-[#92723A] bg-white/90 backdrop-blur-md rounded-lg">
            ...
            <h3 className="text-lg font-medium font-cairo mb-2">
            ...
            <div className="flex-1 mr-10">
              ...
              >
                {shopInfo.requestStatus === "approved" && user?.paid
                  ...
                  : shopInfo.requestStatus === "rejected"
                  ? `تم رفض طلب التفعيل. ${
                      shopInfo.rejectionReason
                        ? `السبب: ${shopInfo.rejectionReason}`
                        : ""
                    }`
                  : shopInfo.requestStatus === "pending"
                ...
                {shopInfo.requestStatus === "pending" && (
                  <div>
                    <p className="text-xs text-amber-600 mb-2">
                      📄 تم رفع السجل التجاري والمستندات المطلوبة • ستتلقى
                      إشعاراً عند الموافقة
                    </p>
                    <div className="flex gap-2">
                      <Button
                        variant="outline"
                        onClick={() =>
                          navigate(`/shop/activation-request/${shopInfo._id}`)
                        }
                      ...
                      <div className="mr-3">
                        ...
                        <div className="mt-1 text-sm text-amber-700">
                          ...
                          <ul className="list-disc list-inside mr-4 space-y-1">
                            <li
                              className={
                                shopInfo.requestStatus === "approved"
                                  ? "text-green-600"
                                  : "text-amber-700"
                              }
                            >
                              {shopInfo.requestStatus === "approved" ? "" : ""}{" "}
                              موافقة الأدمن على المتجر
                            </li>
                            <li
                              className={
                                user?.paid ? "text-green-600" : "text-amber-700"
                              }
                            >
                ...
                <Button
                  ...
                  onClick={() => {
                    if (shopInfo?.requestStatus === "approved" && user?.paid) {
                      ...
                      alert(
                        "يجب موافقة الأدمن على المتجر وإتمام الدفع أولاً لإضافة المنتجات"
                      );
                    }
                  }}
                  disabled={
                ...
                <Button
                  ...
                  onClick={() => {
                    if (shopInfo?.requestStatus === "approved" && user?.paid) {
                      ...
                      alert(
                        "يجب موافقة الأدمن على المتجر وإتمام الدفع أولاً لإدارة المنتجات"
                      );
                    }
                  }}
                  disabled={
        ...
        {isShopOwner && (
          <Card className="bg-white/90 backdrop-blur-md rounded-lg">
            ...
            <CardContent>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div>
                  <label className="block text-sm font-medium text-[#6D552C] mb-2">
                    التاريخ
                  </label>
                  <input
                    type="date"
                ...
                <div>
                  ...
                  >
                    ...
                    <option value={210}>3 ساعات ونصف</option>
                    <option value={240}>4 ساعات</option>
                    <option value={270}>4 ساعات ونصف</option>
        ...
        <Card className="bg-white/90 backdrop-blur-md rounded-lg">
          <CardHeader>
            <CardTitle className="font-cairo">
              {isShopOwner
                ? `الأوقات المتاحة (${availableTimes.length})`
                : `مواعيدي المحجوزة (${availableTimes.length})`}
            </CardTitle>
          </CardHeader>
          <CardContent>
            {availableTimes.length > 0 ? (
              <div className="space-y-3">
                {availableTimes.map((timeSlot) => (
                  <div
                    key={timeSlot._id}
                    className="flex items-center justify-between p-4 border border-gray-200 rounded-lg bg-white/80"
                  >
                      ...
                      {!isShopOwner && (
                        <div className="flex items-center space-x-2">
                          ...
                          <span className="font-medium">
                            {timeSlot.shop?.name || "Unknown Shop"}
                          </span>
                        </div>
                      )}
                      <div className="flex items-center space-x-2">
              ...
              <div className="text-center py-8 text-[#92723A] bg-white/80 rounded-lg">
                ...
                <p className="text-sm">
                  {isShopOwner
                    ? "أضف أوقات متاحة للعملاء للحجز"
                    : "ابدأ بحجز المواعيد في المتاجر"}
                </p>
              </div>
            )}
          </CardContent>
  ...
  const QRCodeTab = () => {
    ...
    const generateNewQRCode = async () => {
      ...
      try {
        setQrCodeLoading(true);
        const response = await shopService.generateQRCode(
          shopInfo._id || shopInfo.id
        );
        setQrCode(response.data);
    ...
    return (
      <div className="min-h-screen bg-white flex items-center justify-center pt-20">
        <div className="max-w-md mx-auto text-center">
          ...
          <h1 className="text-3xl font-bold font-cairo text-[#8A5700] mb-4">
            تم رفض الوصول
          </h1>
          <p className="text-lg font-tajawal text-[#A66A00] mb-8">
            الوصول إلى لوحة التحكم مقتصر على أصحاب المتاجر والمديرين فقط.
          </p>
          <Button
            onClick={() => navigate("/")}
    ...
    <div className="min-h-screen bg-white pt-20 w-full font-cairo">
      <div className="flex w-full">
        ...
        <div className="flex-1 px-4 sm:px-6 lg:px-8 py-8 lg:pr-80">
          <div className="mb-6">
            <h1 className="text-3xl font-bold font-cairo text-primary-900">
              {t("dashboard.welcome", { name: user?.name || "User" })}
            </h1>
            <p className="text-secondary-800 mt-1 font-cairo">
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <button
                    ...
                    className={`flex items-center space-x-2 px-3 py-2 rounded-lg font-medium text-xs whitespace-nowrap transition-all duration-300 ${
                      activeTab === tab.id
        ...
        <div className="hidden lg:block fixed right-0 top-20 h-[calc(100vh-5rem)] w-72 bg-white/90 backdrop-blur-md shadow border-l border-gray-200">
          <div className="p-6">
            <h2 className="text-xl font-bold font-cairo text-[#8A5700] m-6">
              قائمة لوحة التحكم
            </h2>
              ...
              {tabs.map((tab) => {
                ...
                return (
                  <button
                    key={tab.id}
                    onClick={() => setActiveTab(tab.id)}
                    className={`w-full flex items-center space-x-3 px-4 py-3 rounded-lg font-medium text-sm transition-all duration-300 text-left ${
                      activeTab === tab.id
                        ? "bg-gradient-to-r from-[#C37C00] to-[#A66A00] text-white shadow-md"
                        : "text-[#A66A00] hover:bg-[#FFF8E6] hover:text-[#8A5700] hover:shadow-sm"
                    }`}
