...
const ProductList = () => {
  ...
  const ProductCard = ({ product, isListView = false }) => {
    ...
    return (
      <Card
        className={`group hover:shadow-xl transition-all duration-700 cursor-pointer border-2 border-secondary-2 hover:border-primary-500 hover:shadow-primary-500/25 transform hover:-translate-y-1 sm:hover:-translate-y-2 lg:hover:-translate-y-4 hover:scale-105 ${
          isListView
            ? "flex flex-col sm:flex-row h-auto sm:h-80"
      ...
      >
        {/* Container الصورة المحسّن */}
        <div
          className={`relative overflow-hidden ${
            isListView
              ? "w-full sm:w-80 flex-shrink-0 rounded-t-2xl sm:rounded-l-3xl sm:rounded-tr-none"
              : "w-full rounded-t-2xl sm:rounded-t-3xl"
          }`}
        >
          <div
            className={`relative ${
              isListView ? "h-64 sm:h-full" : "h-56 sm:h-64 lg:h-72 xl:h-80" // زيادة ارتفاع الصورة بشكل كبير
            } overflow-hidden`}
          >
            <img
              ...
              style={{ imageRendering: 'crisp-edges' }} // تحسين جودة الصورة
              onError={(e) => {
        ...
        >
          {/* Main content */}
          <div className="flex-1 mb-6 sm:mb-2"> {/* زيادة المسافة قبل الفوتر */}
            <h3 className="font-bold text-lg sm:text-xl mb-3 group-hover:text-yellow-600 transition-colors line-clamp-2 leading-tight group-hover:scale-105 transform origin-right duration-300">
              {safeProduct.name}
            </h3>
            ...
            <div className="mb-3 sm:mb-4">
              <div className="flex items-center gap-2">
                <span className="text-xl sm:text-2xl lg:text-3xl font-bold text-[#C37C00] group-hover:text-[#A66A00] transition-colors duration-300">
            ...
            {safeProduct.shopName &&
              safeProduct.shopName !== "متجر غير معروف" && (
                // <div className="flex items-center gap-3 mb-3 bg-blue-50 p-3 rounded-xl border border-blue-200/50">
                <div className="flex items-center gap-3  bg-blue-50 p-3 rounded-xl border border-blue-200/50">
                  <div className="w-5 h-5 sm:w-7 sm:h-7 bg-gradient-to-r from-[#C37C00] to-[#A66A00] rounded-full flex items-center justify-center">
                    <ShoppingBag className="w-3 h-3 sm:w-4 sm:h-4 text-white" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-xs sm:text-sm text-gray-600 font-medium">متاح في</p>
                    <p
                      className="text-sm sm:text-base font-bold text-[#C37C00] hover:text-[#A66A00] cursor-pointer transition-colors duration-200 truncate"
                      onClick={(e) => {
                        e.stopPropagation();
          ...
          {/* Footer area مع مسافة كافية */}
          <div className="mt-auto pt-4 border-t border-gray-100"> {/* إضافة حد علوي للفصل */}
            <div className="flex flex-col gap-3">
              <Button
                size="lg"
                className="w-full bg-gradient-to-r from-[#C37C00] via-[#A66A00] to-[#8A5700] hover:from-[#A66A00] hover:via-[#8A5700] hover:to-[#6D4500] text-white px-4 sm:px-6 py-3 sm:py-4 rounded-xl sm:rounded-2xl hover:shadow-xl transition-all duration-300 transform hover:scale-105 font-semibold text-sm sm:text-base border border-[#C37C00]/50"
                onClick={(e) => {
                  e.stopPropagation();
                  navigate(ROUTES.PRODUCT_DETAILS(productId));
                }}
